    ...
    while not done:
        ...
        if ch.lower() == 'stop':
            ...
            if len(word):
                ...
                tts.play_response("Atleast make a move.")
                continue
        elif  len(ch) != 1 and len(word)>0:
            print("Invalid command\n")
            tts.play_response("Invalid command.")
            continue
        word = word + ch.lower()
        word = bot.play(word)
    if bored:
        try:
            tts.play_response("Say 'Next' to continue.")
            nex = stt.get_transcript()
        except:
